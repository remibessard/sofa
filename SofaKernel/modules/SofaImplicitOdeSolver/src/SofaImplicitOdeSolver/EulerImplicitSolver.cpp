...
void EulerImplicitSolver::solve(const core::ExecParams* params, SReal dt, sofa::core::MultiVecCoordId xResult, sofa::core::MultiVecDerivId vResult)
{
	...
	sofa::helper::AdvancedTimer::stepBegin("inform constraint");
    /// inform the constraint parameters about the position and velocity id
    mop.cparams.setX(xResult);
    mop.cparams.setV(vResult);

	sofa::helper::AdvancedTimer::stepNext("inform constraint", "allocation");
    ...
    sofa::helper::AdvancedTimer::stepNext("allocation","ComputeForce");
	...
	if (firstOrder)
		matrix = MechanicalMatrix::K * (-h) + MechanicalMatrix::M;// MechanicalMatrix(1, 0, -h * tr); //MechanicalMatrix::K * (-h*tr) + MechanicalMatrix::M;
    else
		...
		//MechanicalMatrix(1+tr*h*f_rayleighMass.getValue(),-tr*h,-tr*h*(h+f_rayleighStiffness.getValue())); // MechanicalMatrix::K * (-tr*h*(h+f_rayleighStiffness.getValue())) + MechanicalMatrix::B * (-tr*h) + MechanicalMatrix::M * (1+tr*h*f_rayleighMass.getValue());
	...
	sofa::helper::AdvancedTimer::stepBegin("Reste");
...
#undef SOFATIMER_NEXTSTEP
	...
	sofa::helper::AdvancedTimer::stepEnd("Reste");
